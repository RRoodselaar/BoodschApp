...
$(function () {
    ...
    function fillTransactions(el) {
        var uId = $(el).attr('id');
        var sum = 0.0;
        //var log = '"item.date";"text";"amount"\n';
        var row = $.map(items, function(item) {
            var amount = 0.0;
            if (item.userId == uId) amount += parseFloat(item.amount);
            if (item.users.indexOf(uId) > -1) amount -= parseFloat(item.amount) / item.users.split(',').length;
            //log += '"'+item.date.getDate()+'-'+(item.date.getMonth()+1)+'-'+item.date.getFullYear()+'";"'+item.text+'";'+amount.toFixed(2)+'\n';
            if (amount != 0) {
                sum += amount;
                return $('<li>').css('width','100%')
                    .append($('<div>').css('clear','none').css('float','left').css('width','90px').append(item.date.getDate()+'-'+(item.date.getMonth()+1)+'-'+item.date.getFullYear()).css('text-align','right'))
                    .append($('<div>').css('clear','none').css('float','left').css('width','auto').append(item.text))
                    .append($('<div>').css('clear','none').css('float','right').css('width','80px').append("&euro; " + amount.toFixed(2)).css('text-align','right'));
            }
        });
        var totalrow = "";
        if (sum != 0) totalrow = $('<li>').css('font-weight','bold').css('width','100%')
            .append($('<div>').css('float','left').css('width','90px').append('&nbsp;'))
    ...
    function getTransactions(el) {
        if (!items) ItemTable.orderByDescending('date').take(1000).read().then( function (t) {
...
>>>>>>> fbf971d96ea0e336b23a372044e48f8c0832c687
});
